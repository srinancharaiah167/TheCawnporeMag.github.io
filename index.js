...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Search bar code
  // The search bar in the "Issues" section
  const issueSearchInput = document.getElementById("issueSearch");
  const issueSearchButton = document.getElementById("search-button");

  function searchIssues() {
    const query = issueSearchInput.value.toLowerCase().trim();
    const issueBoxes = document.querySelectorAll(".issue-box");
    let resultsFound = false;

    issueBoxes.forEach((box) => {
      const issueNumber = box
        .querySelector(".issuecover")
        .getAttribute("data-issue-number")
        .toLowerCase();
      const issueTitle = box.querySelector("h3").textContent.toLowerCase();

      if (issueNumber.includes(query) || issueTitle.includes(query)) {
  ...
  issueSearchButton.addEventListener("click", searchIssues);

  // Add an event listener to the input field for live search as well
  issueSearchInput.addEventListener("input", searchIssues);

  // Other search bar code (for team members, etc.) - keeping this here just in case it's used elsewhere
